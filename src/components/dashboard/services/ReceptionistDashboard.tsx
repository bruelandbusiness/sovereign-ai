...
const SENTIMENT_COLORS: Record<string, string> = {
  ...
  neutral: "bg-muted text-foreground/80 border-border/30",
  ...
  return (
    <>
      ...
      >
        ...
        <td className="p-3">
          <span
            className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${STATUS_COLORS[call.status] ?? "bg-muted text-muted-foreground"}`}
          >
            {call.isEmergency && (
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto rounded-lg border border-white/[0.06] p-3 bg-background">
                    {turns.map((turn, i) => (
                      <div
                        key={i}
                        className={`text-sm ${turn.role === "user" ? "text-blue-300" : "text-foreground/80"}`}
                      >
