...
import { Users, Trophy, RotateCcw, Calendar, AlertCircle, DollarSign } from "lucide-react";
...
function formatRevenueLost(leadsLost: number, vertical?: string): string {
  const avgValue = (vertical && AVG_JOB_VALUE[vertical]) || 350;
  const revenue = leadsLost * avgValue;
...
export function AuditResults({ result, onReset }: AuditResultsProps) {
  ...
  const estimatedRevenueLost = formatRevenueLost(result.estimated_leads_lost, result.vertical);
        ...
        <FadeInView delay={0.2}>
          ...
          >
            <motion.div
              ...
              className="flex flex-col items-center gap-2 rounded-xl border border-border/50 glass-card p-5"
            >
            ...
            <div className="mx-auto mt-12 max-w-2xl">
              ...
              <div className="flex flex-col gap-3">
                {priorityActions.map((action, i) => (
                  <div
                    key={`${action.title}-${i}`}
                    className="flex items-start gap-4 rounded-xl border border-border/50 glass-card p-4"
                  >
          ...
          <div className="mx-auto mt-16 max-w-xl rounded-2xl border border-primary/20 bg-primary/5 p-8 text-center sm:p-10">
            ...
            <GradientButton
              ...
              onClick={() => {
                ...
                window.open(calendlyUrl || "/strategy-call", "_blank");
              }}
            >
