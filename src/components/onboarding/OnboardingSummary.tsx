...
import { ArrowLeft, CheckCircle2, Loader2, ShieldCheck, Clock, Rocket, BarChart3, AlertCircle } from "lucide-react";
...
export function OnboardingSummary({
  ...
  // Check if the selected services match a bundle for discounted pricing
  const matchedBundle = useMemo(() => {
    const selected = new Set(formData.step3.selectedServices);
  ...
  const estimatedTotal = matchedBundle ? matchedBundle.price : individualTotal;
      ...
      <div className="rounded-lg border border-border p-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="mt-2 rounded-lg border border-primary/20 bg-gradient-to-r from-primary/10 to-emerald-500/10 p-4">
            {matchedBundle && individualTotal > matchedBundle.price && (
              <div className="mb-2 flex items-center justify-between text-sm text-muted-foreground">
                <span>Individual pricing</span>
                <span className="line-through">{formatPrice(individualTotal)}/mo</span>
              </div>
            )}
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold">
                {matchedBundle ? `${matchedBundle.name} Bundle Price` : "Estimated Monthly Total"}
              </span>
            ...
            {matchedBundle && individualTotal > matchedBundle.price && (
              <p className="mt-1 text-right text-xs font-medium text-accent">
                You save {formatPrice(individualTotal - matchedBundle.price)}/mo with this bundle
              </p>
            )}
          </div>
        </div>
