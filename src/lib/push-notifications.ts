...
const VAPID_SUBJECT = process.env.VAPID_SUBJECT || "mailto:support@trysovereignai.com";
...
if (!isVapidConfigured && process.env.NODE_ENV === "production") {
  logger.warn(
    "[push-notifications] VAPID keys not configured — push notifications will be skipped. In-app notifications still work."
  );
}
...
export async function sendPushNotification(
  ...
  try {
    ...
    logger.error("web-push package not installed. Run: npm install web-push", {
      error: String(err),
    });
    return { sent: 0, failed: 0 };
  }

  // Safe: isVapidConfigured guard above ensures both keys are defined
