 ...
 */
function generateHowToSchema(serviceInfo: {
  name: string;
  description: string;
...
export async function generateAEOContent(
  ...
  if (!apiKey || !clientId) {
    console.warn("[aeo] ANTHROPIC_API_KEY or clientId not set — returning template content");
...
[content]`,
    ...
    if (err instanceof GovernanceBlockedError) {
      console.warn(`[aeo] Governance blocked for client ${clientId}: ${err.reason}`);
      return templateFallback();
    }
    // Log and re-throw so the caller route can classify the error properly
    console.error(`[aeo] AI call failed for client ${clientId}:`, err instanceof Error ? err.message : err);
    throw err;
  }
