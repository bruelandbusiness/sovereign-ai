 ...
 */
export function getLowestMonthlyPayment(amountCents: number): number {
...
export function formatCentsToDollarsExact(cents: number): string {
...
export async function createApplication(params: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error("Wisetack create application failed", {
    ...
    logger.errorWithCause("Wisetack create application error", error);
    return null;
  }
}
...
export async function checkStatus(
  ...
  try {
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      logger.error("Wisetack check status failed", {
        statusCode: response.status,
        responseBody: errorBody,
        externalId,
      });
    ...
    logger.errorWithCause("Wisetack check status error", error);
    return null;
  }
}
