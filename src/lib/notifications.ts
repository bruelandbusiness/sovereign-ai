...
export async function createNotification(
  ...
  // Also send to Telegram for critical notifications
  try {
    const { sendTelegramAlert, isConfigured } = await import("@/lib/telegram");
    if (isConfigured() && (type === "billing" || urgent)) {
      await sendTelegramAlert("critical", title, message);
    }
  } catch {
