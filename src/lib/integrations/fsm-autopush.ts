  ...
  try {
    ...
    for (const connection of connections) {
      try {
        ...
        }).catch((err) => console.error("[fsm] Lead sync log write failed:", err instanceof Error ? err.message : err));
      }
    }
  } catch (err) {
    console.error("[fsm-autopush] Error querying FSM connections:", err);
  }
}
  ...
  try {
    ...
    for (const connection of connections) {
      try {
        ...
        }).catch((err) => console.error("[fsm] Booking sync log write failed:", err instanceof Error ? err.message : err));
      }
    }
  } catch (err) {
    console.error("[fsm-autopush] Error querying FSM connections:", err);
