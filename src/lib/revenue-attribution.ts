...
/**
 * Get the full attribution chain for a lead (all events in their journey).
 */
...
async function getAttributionChain(leadId: string): Promise<AttributionEvent[]> {
  ...
  return events.map((e) => {
    ...
    if (e.metadata) {
      try {
        meta = JSON.parse(e.metadata) as unknown;
      } catch {
        /* malformed metadata */
      }
    }
