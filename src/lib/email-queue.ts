...
export async function queueEmail(
  ...
  if (bounced) {
    throw new Error(
      "Cannot queue email: recipient address has previously bounced"
    );
  }
...
export async function processEmailQueue(): Promise<{
  ...
  for (let idx = 0; idx < claimedEmails.length; idx++) {
    ...
    // Transactional emails (clientId=null) skip compliance checks.
    if (email.clientId) {
      const complianceCheck = await canSendEmail(
        email.clientId,
        email.to,
        email.html
      );
      if (!complianceCheck.allowed) {
        await prisma.emailQueue.update({
          ...
          data: {
            status: "failed",
            attempts: newAttempts,
            lastError: `Compliance blocked: ${complianceCheck.reason}`,
          },
        });
        failed++;
        continue;
      ...
      // Log successful compliance pass as a contact attempt
    ...
    try {
      ...
      // Classify the error: SendGridError carries retryable flag.
      const isRetryable =
        error instanceof SendGridError ? error.retryable : true;
      const isRateLimited =
        error instanceof SendGridError && error.statusCode === 429;

      if (isRateLimited) {
        const backoffMs = Math.min(1000 * Math.pow(2, email.attempts), 30_000);
        logger.warn("[email-queue] SendGrid rate limit hit — backing off", {
          emailId: email.id,
          backoffMs,
        });
      ...
      // Non-retryable errors (403 auth, 400 bad request) fail immediately
      // regardless of remaining attempts.
      if (!isRetryable || newAttempts >= email.maxAttempts) {
        await prisma.emailQueue.update({
          where: { id: email.id },
          data: {
            status: "failed",
        ...
        if (!isRetryable) {
          logger.error("[email-queue] Permanent send failure — not retrying", {
            emailId: email.id,
            to: email.to,
            error: errorMessage,
          });
