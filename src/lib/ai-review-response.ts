...
const AI_MODEL =
  process.env.CLAUDE_REVIEW_MODEL || "claude-haiku-4-5-20251001";
...
export async function generateReviewResponse(
  ...
  if (!apiKey) {
    logger.warn("[ai-review-response] ANTHROPIC_API_KEY not set — returning template response");
    return getTemplateResponse(review);
  }

  if (!review.clientId) {
    logger.warn("[ai-review-response] No clientId provided — returning template response");
    return getTemplateResponse(review);
  }
...
- Do not use emojis`,
    ...
    if (err instanceof GovernanceBlockedError) {
      logger.warn(`[ai-review-response] Governance blocked for client ${review.clientId}: ${err.reason}`);
      return getTemplateResponse(review);
    }
    ...
    logger.errorWithCause(`[ai-review-response] AI call failed for client ${review.clientId}, falling back to template:`, err instanceof Error ? err.message : err);
    return getTemplateResponse(review);
  }
}
