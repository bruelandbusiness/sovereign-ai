...
(function() {
  ...
  styleEl.textContent = [
    ...
    '#sov-social-proof .sov-sp-notification { pointer-events: auto; background: #1a1a2e; border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 12px 16px; max-width: 320px; box-shadow: 0 8px 32px rgba(0,0,0,0.4); opacity: 0; transform: translateY(20px); transition: opacity 0.4s ease, transform 0.4s ease; display: flex; align-items: center; gap: 12px; }',
    '#sov-social-proof .sov-sp-notification.sov-sp-visible { opacity: 1; transform: translateY(0); }',
    '#sov-social-proof .sov-sp-notification.sov-sp-hiding { opacity: 0; transform: translateY(-10px); }',
    '#sov-social-proof .sov-sp-icon { width: 36px; height: 36px; border-radius: 50%; background: rgba(76,133,255,0.15); display: flex; align-items: center; justify-content: center; flex-shrink: 0; color: #4c85ff; font-size: 16px; }',
    '#sov-social-proof .sov-sp-content { flex: 1; }',
    '#sov-social-proof .sov-sp-title { color: #fff; font-size: 13px; font-weight: 600; line-height: 1.3; }',
    '#sov-social-proof .sov-sp-subtitle { color: #a0a0b8; font-size: 11px; margin-top: 2px; }',
    '#sov-social-proof .sov-sp-close { background: none; border: none; color: #a0a0b8; cursor: pointer; font-size: 14px; padding: 4px; line-height: 1; position: absolute; top: 6px; right: 8px; }',
    '#sov-social-proof .sov-sp-close:hover { color: #fff; }',
    '#sov-social-proof .sov-sp-stars { color: #fbbf24; font-size: 12px; letter-spacing: 1px; }',
    '@media (max-width: 500px) {',
    '  #sov-social-proof { left: 12px; right: 12px; bottom: 12px; }',
    '  #sov-social-proof .sov-sp-notification { max-width: 100%; }',
    '}'
  ].join('\\n');
  ...
  function showNext() {
    ...
    if (item.type === 'review' && item.rating) {
      var safeRating = Math.max(1, Math.min(5, Math.floor(Number(item.rating)) || 1));
      starsHTML = '<div class="sov-sp-stars">' + '\\u2605'.repeat(safeRating) + '</div>';
    }

    notification.innerHTML = [
      '<div class="sov-sp-icon">' + escapeHtml(iconEmoji) + '</div>',
