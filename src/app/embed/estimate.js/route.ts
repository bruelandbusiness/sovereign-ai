...
(function() {
  ...
  style.textContent = [
    ...
    /* -- Upload zone -- */
    '#sov-estimate .sov-upload-zone { border: 2px dashed ' + BORDER + '; border-radius: 12px; padding: 24px; text-align: center; cursor: pointer; transition: border-color 0.2s, background 0.2s; margin-bottom: 14px; }',
    '#sov-estimate .sov-upload-zone:hover, #sov-estimate .sov-upload-zone.dragover { border-color: ' + PRIMARY + '; background: rgba(245,158,11,0.05); }',
    '#sov-estimate .sov-upload-zone p { color: ' + TEXT_MUTED + '; font-size: 14px; }',
    '#sov-estimate .sov-upload-zone .sov-icon { color: ' + PRIMARY + '; margin-bottom: 8px; }',
    '#sov-estimate .sov-upload-zone .sov-filename { color: ' + PRIMARY + '; font-weight: 600; margin-top: 8px; font-size: 13px; }',
    '#sov-estimate .sov-upload-zone .sov-preview { max-width: 200px; max-height: 150px; border-radius: 8px; margin-top: 8px; }',
    ...
    '#sov-estimate .sov-result-card { background: ' + BG_PANEL + '; border: 1px solid ' + BORDER + '; border-radius: 12px; padding: 16px; margin-bottom: 12px; }',
    '#sov-estimate .sov-result-card h3 { color: #fff; font-size: 15px; font-weight: 700; margin-bottom: 8px; }',
    '#sov-estimate .sov-result-card p { color: ' + TEXT_MUTED + '; font-size: 13px; line-height: 1.5; }',
    '#sov-estimate .sov-estimate-range { text-align: center; padding: 20px; background: linear-gradient(135deg, rgba(245,158,11,0.1), rgba(217,119,6,0.1)); border: 1px solid rgba(245,158,11,0.3); border-radius: 12px; margin-bottom: 12px; }',
    '#sov-estimate .sov-estimate-range .sov-price { font-size: 28px; font-weight: 800; color: ' + PRIMARY + '; }',
    '#sov-estimate .sov-estimate-range .sov-label { font-size: 12px; color: ' + TEXT_MUTED + '; margin-top: 4px; }',
    '#sov-estimate .sov-confidence { display: inline-block; padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; }',
    '#sov-estimate .sov-confidence-high { background: rgba(16,185,129,0.15); color: #10b981; }',
    '#sov-estimate .sov-confidence-medium { background: rgba(245,158,11,0.15); color: #f59e0b; }',
    '#sov-estimate .sov-confidence-low { background: rgba(239,68,68,0.15); color: #ef4444; }',
    ...
    /* -- Spinner -- */
    '#sov-estimate .sov-spinner { display: inline-block; width: 20px; height: 20px; border: 2px solid rgba(255,255,255,0.3); border-top-color: #fff; border-radius: 50%; animation: sovSpin 0.6s linear infinite; }',
  ...
  footLink.href = 'https://www.trysovereignai.com';
  ...
  function showResults(data) {
    ...
    var allowedConf = ['low', 'medium', 'high'];
    var rawConf = allowedConf.indexOf(data.confidence) !== -1 ? data.confidence : 'medium';
