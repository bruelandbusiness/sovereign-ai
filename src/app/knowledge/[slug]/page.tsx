...
/**
 * Sanitize HTML output from renderMarkdown. Uses DOMParser to strip
 * dangerous elements (script, iframe, etc.) and event-handler attributes
 * while preserving safe formatting tags produced by the markdown renderer.
 */
function sanitizeRenderedHtml(html: string): string {
  ...
  doc.querySelectorAll(
    "script, iframe, object, embed, form, link[rel=import], meta, base, applet, style"
  ).forEach((el) => el.remove());
  // Remove event handlers, javascript: URLs, and data: URLs
  doc.querySelectorAll("*").forEach((el) => {
    for (const attr of [...el.attributes]) {
      if (
        attr.name.startsWith("on") ||
        (attr.value && /^\s*(javascript|data):/i.test(attr.value))
      ) {
                  ...
                  <CardContent className="p-6 sm:p-8">
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: `<p class="text-sm text-foreground/80 leading-relaxed mb-3">${sanitizeRenderedHtml(renderMarkdown(article.content))}</p>`,
                      }}
                    />
                  </CardContent>
                </Card>
