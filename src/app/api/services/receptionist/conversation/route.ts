...
<Response>
  ...
  <Hangup/>
    ...
    if (userTurnCount > maxTurns) {
      ...
      generateCallSummary(callLogId, history).catch((err) =>
        logger.errorWithCause("Failed to generate call summary", err)
      );
...
<Response>
  ...
  <Hangup/>
    ...
    logger.errorWithCause("[receptionist/conversation] Error:", error);
...
async function generateCallSummary(
  ...
  try {
    ...
    try {
      ...
      if (err instanceof GovernanceBlockedError) {
        logger.warn(`[receptionist/conversation] Summary blocked by governance: ${err.reason}`);
        return;
      }
      throw err;
    }
    ...
    logger.errorWithCause("[receptionist/conversation] Summary generation error:", error);
  }
}
