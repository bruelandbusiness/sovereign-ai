...
Return ONLY the post content, no other text or explanation.`;
    ...
    try {
      ...
      logger.errorWithCause("AI content generation failed:", error);
  ...
  if (body.publishNow) {
    try {
      ...
      logger.errorWithCause("Failed to publish post:", error);
      status = "failed";
    }
...
export async function PUT(request: Request) {
  try {
    ...
    logger.errorWithCause("Social post update error:", error);
    return NextResponse.json(
