...
import { validateTwilioSignature, twilioClient, twilioPhoneNumber, signedRecordingUrl, sendSms } from "@/lib/twilio";
  ...
  try {
    ...
    if (normalizedStatus === "completed" && phoneCall.transcription) {
      try {
        ...
        try {
          ...
          console.error("[voice/status] Failed to add to inbox:", inboxErr);
        }
      } catch (err) {
        console.error("[voice/status] AI summary failed:", err);
      }
    }
    ...
    if (normalizedStatus === "no-answer" || normalizedStatus === "busy") {
      try {
        ...
        console.error("[voice/status] Missed call textback failed:", err);
      }
    }
...
async function handleMissedCallTextback(phoneCall: {
  ...
  if (smsResult.success) {
    ...
    try {
      ...
      console.error("[voice/status] Failed to add textback to inbox:", inboxErr);
    }
    ...
    console.error("[voice/status] SMS send failed:", smsResult.error);
