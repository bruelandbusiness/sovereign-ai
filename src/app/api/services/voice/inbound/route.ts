...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build the greeting and voice config from the client's voice config
    let greeting =
    ...
    if (clientService?.config) {
      try {
        ...
        // Use default greeting and voice config
      }
    }

    const conversationUrl = `${appUrl}/api/services/voice/conversation?callSid=${encodeURIComponent(callSid)}`;

    // Generate TwiML fragments with ElevenLabs or Polly fallback
    const [greetingTwiml, listeningTwiml, goodbyeTwiml] = await Promise.all([
      generateTwimlWithVoice(greeting, voiceConfig),
      generateTwimlWithVoice("I'm listening.", voiceConfig),
      generateTwimlWithVoice("I didn't catch that. Goodbye!", voiceConfig),
    ]);
...
<Response>
  ...
  <Hangup/>
    ...
    logger.errorWithCause("[voice/inbound] Error:", error);
