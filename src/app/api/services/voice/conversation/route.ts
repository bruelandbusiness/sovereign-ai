...
// ---------------------------------------------------------------------------
// Helper: resolve voice config from the client's service config
// ---------------------------------------------------------------------------

async function resolveVoiceConfig(clientId: string): Promise<VoiceConfig> {
  try {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!speechResult || !callSid) {
      const repeatTwiml = await generateTwimlWithVoice("I didn't catch that. Could you please repeat?");
      const goodbyeTwiml = await generateTwimlWithVoice("I still didn't hear anything. Goodbye!");
...
<Response>
  ...
  <Hangup/>
    ...
    if (!phoneCall) {
      const notFoundTwiml = await generateTwimlWithVoice("Sorry, we could not find your call. Goodbye.");
      const twiml = `<?xml version="1.0" encoding="UTF-8"?><Response>${notFoundTwiml}<Hangup/></Response>`;
      return new Response(twiml, {
        headers: { "Content-Type": "text/xml" },
      });
    }

    // Resolve voice config for this client
    const voiceConfig = await resolveVoiceConfig(phoneCall.clientId);
    ...
    if (userTurns > MAX_CONVERSATION_TURNS) {
      ...
      const wrapUpTwiml = await generateTwimlWithVoice(wrapUp, voiceConfig);
      const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  ${wrapUpTwiml}
  <Hangup/>
    ...
    try {
      ...
      if (err instanceof GovernanceBlockedError) {
        ...
        const budgetTwiml = await generateTwimlWithVoice(budgetMsg, voiceConfig);
        const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  ${budgetTwiml}
  <Hangup/>
    ...
    const [replyTwiml, stillThereTwiml, thankYouTwiml] = await Promise.all([
      generateTwimlWithVoice(reply, voiceConfig),
      generateTwimlWithVoice("Are you still there? If you need anything else, just let me know.", voiceConfig),
      generateTwimlWithVoice("Thank you for calling. Goodbye!", voiceConfig),
    ]);
...
<Response>
  ...
  <Hangup/>
    ...
    logger.errorWithCause("[voice/conversation] Error:", error);
