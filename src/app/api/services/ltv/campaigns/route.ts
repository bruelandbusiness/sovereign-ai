...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!name || !vertical || !season || !triggerMonth || !subject || !campaignBody) {
      return NextResponse.json(
        {
          error:
            "name, vertical, season, triggerMonth, subject, and campaignBody are required",
        },
        { status: 400 }
      );
    }
    ...
    const campaign = await prisma.seasonalCampaign.create({
      data: {
        ...
        vertical,
        season,
        triggerMonth: Number(triggerMonth),
        subject,
        body: campaignBody,
