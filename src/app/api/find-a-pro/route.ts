...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = serviceAreas.map((sa) => {
      ...
      // Generate a deterministic mock rating based on client id
      let hash = 0;
      for (const ch of client.id) {
        hash = (hash * 31 + ch.charCodeAt(0)) & 0x7fffffff;
      }
      const rating = 4.5 + (hash % 6) / 10; // 4.5 - 5.0
      const reviewCount = 15 + (hash % 186); // 15 - 200
    ...
    console.error("[find-a-pro] Search error:", error);
    return NextResponse.json(
