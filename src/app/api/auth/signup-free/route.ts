...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = await rateLimitByIP(ip, "signup", 5, {
    ...
    if (existingAccount) {
      ...
      if (existingClient) {
        ...
        try {
          const magicLinkResult = await generateMagicLink(normalizedEmail);
    ...
    // Use APP_URL (server-only env var) to avoid Next.js static replacement
    // of NEXT_PUBLIC_ vars at build time, consistent with other auth routes.
