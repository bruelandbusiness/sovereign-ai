  ...
  try {
    ...
    for (const sub of trialsExpiring3Days) {
      try {
        ...
        const unsubscribeUrl = `${process.env.NEXT_PUBLIC_APP_URL || "https://sovereignai.com"}/api/email/unsubscribe?clientId=${sub.clientId}`;
        await sendEmail(
    ...
    console.error("[cron/trial-expiry] Fatal error:", err);
    return NextResponse.json(
