...
export async function POST(
  ...
  try {
    ...
    switch (template.category) {
      case "email_sequence": {
        // content is an array of email objects — batch insert
        const emails = Array.isArray(content) ? content : [];
        if (emails.length > 0) {
          const { count } = await prisma.emailCampaign.createMany({
            data: emails.map((email: Record<string, string>) => ({
              clientId,
          ...
          result = { type: "email_sequence", count };
        } else {
          result = { type: "email_sequence", count: 0 };
        }
        break;
      }

      case "social_calendar": {
        // content is an array of post objects — batch insert
        const posts = Array.isArray(content) ? content : [];
        const now = new Date();
        if (posts.length > 0) {
          const { count } = await prisma.socialPost.createMany({
            data: posts.map((post: Record<string, string>, i: number) => ({
              clientId,
          ...
          result = { type: "social_calendar", count };
        } else {
          result = { type: "social_calendar", count: 0 };
        }
        break;
      }
    ...
    logger.errorWithCause("Template apply error:", err);
