...
import { exportCSV, type ColumnDefinition } from "@/lib/csv-export";
...
export default function CRMPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const columns: ColumnDefinition[] = [
      ...
      { key: "value", header: "Value", format: (v) => v ? `${((v as number) / 100).toFixed(2)}` : "" },
      { key: "date", header: "Created Date" },
    ];
    exportCSV(filtered as unknown as Record<string, unknown>[], columns as unknown as ColumnDefinition<Record<string, unknown>>[], "crm-leads");
  }, [filtered]);
  ...
  // Ref for the kanban board container to enable arrow-key navigation
  const boardRef = useRef<HTMLDivElement>(null);

  /**
   * Keyboard handler for kanban board cards.
   * - ArrowLeft / ArrowRight: move focus to the same-index card in the
   *   adjacent column (or wrap to first/last card if index overflows).
   * - ArrowUp / ArrowDown: move focus within the same column.
   * - Enter: expand/collapse the focused card.
   * - Space: toggle selection of the focused card.
   */
  const handleCardKeyDown = useCallback(
    (e: React.KeyboardEvent, lead: CRMLead, colIndex: number) => {
      ...
      // Get all focusable cards in the current column
      const currentCol = columns[colIndex];
      if (!currentCol) return;
      const cardsInCol = Array.from(
        currentCol.querySelectorAll<HTMLElement>("[data-kanban-card]")
      );
      const cardIndex = cardsInCol.indexOf(e.currentTarget as HTMLElement);
      ...
      switch (e.key) {
        case "ArrowRight": {
          ...
          const nextCards = Array.from(
            nextCol.querySelectorAll<HTMLElement>("[data-kanban-card]")
          );
        ...
        case "ArrowLeft": {
          ...
          const prevCards = Array.from(
            prevCol.querySelectorAll<HTMLElement>("[data-kanban-card]")
          );
        ...
        case "ArrowDown": {
          e.preventDefault();
          const next = cardsInCol[cardIndex + 1];
          if (next) next.focus();
          break;
        }
        case "ArrowUp": {
          e.preventDefault();
          const prev = cardsInCol[cardIndex - 1];
          if (prev) prev.focus();
          break;
        }
        case "Enter": {
        ...
        <main className="flex flex-1 items-center justify-center" role="alert">
          <div className="text-center max-w-md px-4">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Something went wrong fetching your lead data. Please try again later.
            </p>
    ...
    <div className="flex min-h-screen flex-col bg-background page-enter">
      ...
      <main className="flex-1 py-8">
        <Container>
          ...
          <div className="flex items-center gap-3 mb-6">
            <Link href="/dashboard">
              <Button variant="ghost" size="sm" className="min-h-[44px]">
                ...
                <span className="sm:hidden sr-only">Back to Dashboard</span>
              </Button>
            </Link>
          ...
          {/* Stats bar */}
          <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-4 mb-6" role="region" aria-label="Pipeline statistics">
            <Card className="border-white/[0.06] card-interactive">
              <CardContent className="p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] sm:text-xs text-muted-foreground">Total Leads</p>
                </div>
              </CardContent>
            </Card>
            <Card className="border-white/[0.06] card-interactive">
              <CardContent className="p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] sm:text-xs text-muted-foreground">Avg Score</p>
                </div>
              </CardContent>
            </Card>
            <Card className="border-white/[0.06] card-interactive">
              <CardContent className="p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-lg sm:text-2xl font-bold">{stats.conversionRate}%</p>
          ...
          {leads && leads.length === 0 && (
            <EmptyState
              icon={Users}
              title="Your AI is generating leads"
              description="Your AI is actively generating leads. First results typically appear within 7 days. Leads from your chatbot, forms, ads, and voice agents will populate your CRM pipeline automatically."
              actionLabel="Back to Dashboard"
              actionHref="/dashboard"
              secondaryLabel="Learn About Lead Sources"
          ...
          <p className="mb-2 text-xs text-muted-foreground sm:hidden">
            Swipe to view all pipeline stages &rarr;
          </p>
          <div className="overflow-x-auto pb-4 -mx-4 px-4 sm:mx-0 sm:px-0" role="region" aria-label="Pipeline board. Use arrow keys to navigate between cards and columns." tabIndex={0}>
            <div ref={boardRef} className="flex gap-4 min-w-[1200px]">
                      ...
                      {columnLeads.map((lead) => {
                        ...
                        return (
                          <div
                            ...
                            tabIndex={0}
                            role="button"
                            aria-label={`${lead.name}, score ${lead.score}, ${STATUS_SR_LABELS[lead.status as LeadStatus] || lead.status}. Use arrow keys to navigate, Enter to expand, Space to select.`}
                            aria-expanded={isExpanded}
                            aria-pressed={isSelected}
                            onKeyDown={(e) => handleCardKeyDown(e, lead, colIndex)}
                          >
                            <Card
                              className={`cursor-pointer border-white/[0.06] transition-all duration-200 hover:border-white/[0.16] hover:shadow-md hover:shadow-black/10 hover:-translate-y-px ${
                                ...
                                <CardContent className="p-3 space-y-3 text-sm">
                                  ...
                                  <div className="pt-2 border-t border-white/[0.06]">
                                    <Link
                                      ...
                                      className="flex items-center gap-2 rounded-md px-2.5 py-2 text-xs font-medium text-teal-400 transition-colors hover:bg-teal-500/10"
                                    >
                                      <Calendar className="h-3.5 w-3.5" aria-hidden="true" />
